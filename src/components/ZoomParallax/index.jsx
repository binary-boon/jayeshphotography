...
export default function Index() {
    ...
    // Different visibility thresholds for mobile and desktop
    const textOpacity = useTransform(
        scale4, 
        isMobile ? [1.2, 1.8] : [2.5, 3], 
        [0, 1]
    );
    
    const textVisibility = useTransform(
        scale4, 
        isMobile ? [1.2, 1.5] : [2.5, 2.8], 
        ['hidden', 'visible']
    );
                    ...
                    {pictures.map(({ src, scale }, index) => (
                        <motion.div key={index} style={{ scale }} className={styles.el}>
                            <div className={styles.imageContainer}>
                                ...
                                {index === 0 && (
                                    <motion.div
                                        className={`${styles.overlayText} ${isMobile ? styles.mobileOverlay : ''}`}
                                        style={{ 
                                            opacity: textOpacity, 
